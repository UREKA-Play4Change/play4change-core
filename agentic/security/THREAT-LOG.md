...
| R18 | A07 Auth Failures | `HttpAuthRepository.logout()` clears local token storage before the server revoke call. If the network request fails, the server-side refresh token remains valid for up to 7 days. A stolen device could re-authenticate using the still-valid refresh token from that window. | Low | ACCEPTED | N/A | Accepted per P001 F-06. The Keychain is protected by iOS Data Protection (`kSecAttrAccessibleAfterFirstUnlock`); extraction requires physical device bypass. The 7-day window is bounded by the refresh token TTL. Local-first clear is intentional to ensure the user is always logged out on their own device even when offline. |
