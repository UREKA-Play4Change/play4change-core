...
data class TopicResponse(
    ...
    companion object {
        fun from(topic: Topic): TopicResponse = from(topic, null, null)

        fun from(detail: TopicDetail): TopicResponse =
            from(detail.topic, detail.generationLog, detail.stats)

        private fun from(topic: Topic, log: List<TopicPhaseLog>?, stats: TopicStats?): TopicResponse =
            TopicResponse(
                id = topic.id,
                title = topic.title,
